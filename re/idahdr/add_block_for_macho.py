...
def get_block_call():
    ...
    # we tranverse import table and finally get two functions, which exist in libSystem.B.dylib
    ...
    for i in xrange(0, nimps):
        ...
        if name.find("libSystem.B.dylib") != -1:
            ...
            break
    # then we find all xrefs to these two functions
...
def extract_stackblock(addr, bptreg=None):
    ...
    FLAGS_OFF = idc.GetMemberOffset(id_Block_layout, "flags")
    INVOKE_OFF = idc.GetMemberOffset(id_Block_layout, "invoke")
    DESCRIPTOR_OFF = idc.GetMemberOffset(id_Block_layout, "descriptor")

    func = idaapi.decompile(addr)

    blk_var_id = None
    for i, var in enumerate(func.lvars):
        if var.name == new_name:
            blk_var_id = i
            break
    
    if blk_var_id is None:
        return "BLK VAR ID IS NONE!"

    class visitor(idaapi.ctree_visitor_t):
        def __init__(self, var_id, var):
        ...
        def extract_rhs(self, expr):
            x = expr
            while x.op == idaapi.cot_cast or x.op == idaapi.cot_ref:
                x = x.x

            if x.op == idaapi.cot_num:
                return x.n.value(idaapi.tinfo_t.get_stock(idaapi.STI_PCHAR))
            if x.op == idaapi.cot_obj:
                return x.obj_ea

            return None

        def extract_lhs(self, expr):
            x = expr
            while x is not None:
                if x.op == idaapi.cot_ptr:
                    x = x.x
                if x.op == idaapi.cot_cast:
                    x = x.x
                if x.op == idaapi.cot_ref:
                    x = x.x
                if x.op == idaapi.cot_memref:
                    break

                # print("Warn: Can't extract lhs: ", x.opname)
                x = None
            ...
            mem_off = x.m
            if x.x.op == idaapi.cot_var and x.x.v.idx == self.var_id:
                return mem_off

        def visit_expr(self, expr):
            # print('visiting ', hex(expr.ea))
            if idaapi.is_assignment(expr.op): 
                lhs_off = self.extract_lhs(expr.x)
                if lhs_off is None:
                    return 0

                if lhs_off == FLAGS_OFF and expr.y.op == idaapi.cot_num:
                    des = idaapi.idc_parse_decl(idaapi.cvar.idati, "_Block_Flag;", idaapi.TINFO_DEFINITE)[1]
                    t = idaapi.tinfo_t()
                    t.deserialize(None, des, None)
                    expr.y.type = t
                    
                rhs_val = self.extract_rhs(expr.y)
                if rhs_val is None:
                    return 0

                self.var[lhs_off] = rhs_val

            return 0

    var = {}
    v = visitor(blk_var_id, var)
    v.apply_to(func.body, None)
    # get structure member data from decompiled source
    funcaddr = var.get(INVOKE_OFF, 0)
    descriaddr = var.get(DESCRIPTOR_OFF, 0)
    flag = var.get(FLAGS_OFF, 0)
    ...
    #             if funcaddr == 0:
    #                 print "invoke line: %s" % line
    #                 match = re.search(r'invoke = (\(.*\))?([0-9a-zA-Z_]+);', line)
    #                 try:
    #                     funcaddr = idc.LocByName(match.group(2))
    #                     if funcaddr == 0xffffffffffffffff or funcaddr == -1:
    #                         funcaddr = idc.LocByName('_' + match.group(2))
    #                     if funcaddr == 0xffffffffffffffff or funcaddr == -1:
    #                         return "Cant resolve name %s" % match.group(2)
    #                     if isarm:
    #                         funcaddr &= 0xfffffffffffffffe
    #                 except:
    #                     funcaddr = 0
    ...
    #             if descriaddr == 0:
    #                 print "descriptor line: %s" % line
    #                 match = re.search(r'descriptor = (\(.*\))?&?([0-9a-zA-Z_]+);', line)
    #                 try:
    #                     descriaddr = idc.LocByName(match.group(2))
    #                     if descriaddr == 0xffffffffffffffff or descriaddr == -1:
    #                         return "Cant resolve name %s" % match.group(2)
    #                 except:
    #                     descriaddr = 0

    if funcaddr == 0 or flag == 0 or descriaddr == 0:
        ret = "unhandled %x -- funcaddr %x flag %x descriaddr %x" % (addr, funcaddr, flag, descriaddr)
        return ret
